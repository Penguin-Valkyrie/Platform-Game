...
class Player():
    ...
    def update(self, platforms):              
        ...
        if self.jumping:
            ...
            for p in platforms:
                if collision_detection(self, p, 2, screen, True, self.collision_buffer):
                    self.can_jump = True
...
while not game_over:
    ...
    for c in coins:
        c.update()
        c.draw(screen)
        if collision_detection(player, c, 2, screen, False, player.collision_buffer, c.collision_buffer):
            coins.remove(c)
